...
import "../../src/mocks/MockSender.sol";
contract MockSenderBS {
    ...
    function WETH_ADDRESS() public  startPrank stop returns (address payable) {
        return MockSender(proxiedContract).WETH_ADDRESS();
    }

	function sendEthOrWeth(address to, uint256 value) public  startPrank stop  {
        MockSender(proxiedContract).sendEthOrWeth(to, value);
    }
}
